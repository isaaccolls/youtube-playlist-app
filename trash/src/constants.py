playlistsForMusic = [
    # 🎵-all
    'PL_8z4vyyerkO0OsXupspGzzw3-XSjC-01'
]

checkInPlaylists = [
    #  🍭-lolliPop
    'PL_8z4vyyerkNdojRqG-N2Z2m_Y7MiRuur',
    # 💚💛❤️-reggae
    'PL_8z4vyyerkP5JGvkDLUaOjQr5mP3bP2L',
    # 🍁-420
    'PL_8z4vyyerkOwxNjLjnL2apDSjerK0-aj',
    # 🎻-classic
    'PL_8z4vyyerkMCRjGiM_gwSqQyTjm6uuUd',
    # 🥫-salsa
    'PL_8z4vyyerkOA2L_3AUOYQz5PYDYj-cSa',
    # 🐶-htv
    'PL_8z4vyyerkOD1YoaRZvjfmMZy1MsOMtD',
    # ⚡-electro
    'PL_8z4vyyerkMzbRSUbq6D38pNtTBwN9xw',
    # 🔫-hiphop
    'PL_8z4vyyerkOU9rlgZheQg22yAPu5qHHp',
    # 🤘-rock
    'PL_8z4vyyerkMh5swg0FKbTFQRujtlhmr-',
    # 🥷-cumbia
    'PL_8z4vyyerkPHIqxrDUgLA3_TQbVdpzYJ',
    # 🍦-15
    'PL_8z4vyyerkOBt3Lt5iL2VWx04_19yDiq',
    # 🪗-quickly
    'PL_8z4vyyerkPdORFMt2Byu0YWyVGmdZQ_',
    # 🎷-lisa
    'PL_8z4vyyerkP8n-N0abjSVsC5ruSdQbBO',
    # 🫏-llaneritas
    'PL_8z4vyyerkO1ZeGiarPZXKnO84s6I1mx',
    # 🎅-gaitas
    'PL_8z4vyyerkN7ana4SWty5McFf3UxMWek',
]

playlistsForVideo = [
    # 📺⚡-videoElectro
    'PL_8z4vyyerkMhM-RSZkTpq0zELxXTVyzq',
    # 📺🐶-videoMixHtv
    'PL_8z4vyyerkNOgsNi67YVJrfMxaMHiL2u',
    # 📺💚💛❤️-videoReggae
    'PL_8z4vyyerkPaPo2QUvvJSwNpxg-XC4CN',
    # 📺🧘‍♀️-videoLofi
    'PL_8z4vyyerkOoUnuG_8tPiJGcqKSDeko9',
    # 📺🤘-videoRock
    'PL_8z4vyyerkNSD4Q6bBCEqv5OCBHS2LH9',
    # 📺🔫-videoHiphop
    'PL_8z4vyyerkNuVi_SfPch16lyqJFl6xn_'
]
